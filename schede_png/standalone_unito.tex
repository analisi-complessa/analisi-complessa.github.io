...
\newcommand{\qedd}{\tag*{$\square$}} %qed per ambienti matemastici
...
\newcommand{\stampairiferimenti}{%Questo comando va lanciato alla fine, e stampa tutti i riferimenti necessari
	...
	\cleardoublepage
	\stepcounter{capitoloeccolo}\section*{Riferimenti in bibliografia}
	\begin{itemize}
...
\newcommand{\days}[1]{\marginnote{$\spadesuit$}\renewcommand{\datainizion}{#1}}
%%%%%%%%
...
% #4 STRINGA UNIVOCA
	...
	\fi
	\ifx&#1&%
		\paragraph{({\thetotale})} #3% #1 is empty
		\ifx&#2&%
			% #2 is empty
			\else%
			\riferimento{\hyperref[totale#4]{({\thetotale})}}{#2}% #2 is nonempty
			\fi
		\else%
		% #1 nonempty
		\paragraph{#1. ({\thetotale})} #3
		\ifx&#2&%
			% #2 is empty
			\else%
...
\newcommand{\dimframmento}[2]{
	\ifthenelse{\theappendixever = 0}{%
	\paragraph{\hyperref[totale#1]{\textit{Dimostrazione. (\arabic{frammentoB#1})}}} #2}{%
	\paragraph{\hyperref[totale#1]{\textit{Dimostrazione. (\arabic{frammentoB#1})}}} #2
	}
}
\newcommand{\solframmento}[2]{
	\ifthenelse{\theappendixever = 0}{%
	\paragraph{\hyperref[totale#1]{\textit{Soluzione. (\arabic{frammentoB#1})}}} #2}{%
	\paragraph{\hyperref[totale#1]{\textit{Soluzione. (\arabic{frammentoB#1})}}} #2
	}
}
...
\newcommand{\esercizio}[2]{
	...
	\paragraph{\textit{Soluzione ({\thetotale}).}} #2
}
...
\newcommand{\dimostrazione}[2]{\paragraph{\textit{\hyperref[thm:#1]{Dimostrazione \Roman{thm#1}.}}} #2}
\newcommand{\teoref}[1]{$\bigstar$\footnote{Vedi sotto}}
...
%\usepackage[% 
%textwidth=360pt,textheight=595pt]{geometry}
